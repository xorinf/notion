...
function BoardView() {
  ...
  const loading = useBoard(state => state.loading)
  const getBoardById = useBoard(state => state.getBoardById)
  const createList = useBoard(state => state.createList)
  const updateList = useBoard(state => state.updateList)
  const deleteList = useBoard(state => state.deleteList)
  const archiveList = useBoard(state => state.archiveList)
  const reorderList = useBoard(state => state.reorderList)
  const createCard = useBoard(state => state.createCard)
  const deleteCard = useBoard(state => state.deleteCard)
  const reorderCard = useBoard(state => state.reorderCard)
  ...
  const handleCardUpdated = async () => {
    // Re-fetch board to get fresh data after card modal edits
    ...
    <div className="flex-1 flex flex-col h-screen overflow-hidden">
      ...
      <div className="shrink-0 px-6 py-4 border-b border-[#dadce0] bg-white flex items-center gap-4">
        ...
        <div className="min-w-0 flex-1">
          <h1 className="text-xl font-semibold text-[#1d1d1f] tracking-tight truncate">
            {currentBoard.title}
          </h1>
          {currentBoard.description && (
            <p className="text-xs text-[#5f6368] truncate">{currentBoard.description}</p>
          )}
        </div>
            ...
            {(provided) => (
              ...
              >
                  ...
                  <Draggable key={list._id} draggableId={list._id} index={index}>
                    {(provided, snapshot) => (
                      ...
                      >
                        {/* List Header */}
                        <div 
                          className="px-4 py-3 flex items-center justify-between border-b border-[#f1f3f4]"
                ...
                {editingListId === list._id ? (
                  ...
                  <h3
                    className="text-sm font-semibold text-[#1d1d1f] truncate flex-1 cursor-pointer"
                    onDoubleClick={() => { setEditingListId(list._id); setEditListTitle(list.title) }}
                  >
                    {list.title}
                    <span className="ml-2 text-xs font-normal text-[#80868b]">
                      {list.cards?.length || 0}
                    </span>
                  </h3>
                )}
                <div className="relative">
                  <button
                    onClick={() => setListMenuId(listMenuId === list._id ? null : list._id)}
                  ...
                  {listMenuId === list._id && (
                    <>
                      ...
                      <div className="absolute right-0 top-full mt-1 bg-white rounded-xl shadow-lg border border-[#dadce0] py-1 z-20 w-40">
                        <button
                          onClick={() => { setEditingListId(list._id); setEditListTitle(list.title); setListMenuId(null) }}
                          className="w-full flex items-center gap-2 px-3 py-2 text-sm text-[#1d1d1f] hover:bg-[#f1f3f4]"
                        >
                          <Edit2 className="w-3.5 h-3.5" /> Rename
                        </button>
                        <button
                          onClick={async () => { await archiveList(list._id); setListMenuId(null) }}
                          className="w-full flex items-center gap-2 px-3 py-2 text-sm text-[#5f6368] hover:bg-[#f1f3f4]"
                        >
                          <Archive className="w-3.5 h-3.5" /> Archive
                        </button>
                        <button
                          onClick={async () => { await deleteList(list._id); setListMenuId(null) }}
              ...
              <Droppable droppableId={list._id} type="card">
                {(provided, snapshot) => (
                  <div 
                    className={`flex-1 overflow-y-auto px-3 py-2 ${snapshot.isDraggingOver ? 'bg-[#f1f3f4]/50' : ''}`}
                    ref={provided.innerRef}
                    {...provided.droppableProps}
                  >
                            ...
                            {card.members?.length > 0 && (
                              <div className="flex -space-x-1 mt-2">
                                {card.members.slice(0, 3).map((m) => (
                                  <div
                                    key={m._id}
                                    className="w-5 h-5 rounded-full bg-[#1a73e8]/10 text-[#1a73e8] flex items-center justify-center text-[8px] font-bold border border-white"
                                  >
                ...
                {addingCardToList === list._id ? (
                  <form onSubmit={(e) => handleCreateCard(e, list._id)} className="space-y-2 mb-2">
                    ...
                    <div className="flex gap-1">
                      ...
                      <button type="button" onClick={() => { setAddingCardToList(null); setNewCardTitle('') }} className="text-xs text-[#5f6368] px-2 py-1.5 hover:bg-[#f1f3f4] rounded-lg">
            ...
            {showAddList ? (
              <form onSubmit={handleCreateList} className="bg-white rounded-2xl border border-[#dadce0] p-4 shadow-sm space-y-3">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => { setShowAddList(false); setNewListTitle('') }} className="text-xs text-[#5f6368] px-2 py-1.5 hover:bg-[#f1f3f4] rounded-lg">
                    Cancel
                  </button>
                </div>
              </form>
